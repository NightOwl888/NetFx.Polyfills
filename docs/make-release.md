...
This project uses Nerdbank.GitVersioning to assist with creating version numbers based on the current branch and commit. This tool handles making pre-release builds on the main branch and production releases on the release branch.
...
For example, the version.json file on the main branch is currently setup as 4.0.0-alpha.{height}. We want to go from this version to a release of 4.0.0 and set the next version on the main branch as 4.0.1-alpha.{height}.

```console
nbgv prepare-release --nextVersion 4.0.1
```

The command should respond with:

```console
release/v4.0 branch now tracks v4.0.0 stabilization and release.
main branch now tracks v4.0.1-alpha.{height} development.
```

The tool created a release branch named `release/v4.0`. Every build from this branch (regardless of how many commits are added) will be versioned 4.0.0. 
...
```console
nbgv prepare-release beta --nextVersion 4.0.1
```

The command should respond with:

```console
release/v4.0 branch now tracks v4.0.0-beta.{height} stabilization and release.
main branch now tracks v4.0.0-alpha.{height} development.
```

The tool created a release branch named `release/v4.0`. Every build from this branch will be given a unique pre-release version starting with 4.0.0-beta and ending a 4 digit counter. 
...
Then open the `version.json` file at the repository root, and set the `versionHeightOffset` using the formula `versionHeightOffset - ((versionHeight - desiredHeight) + 1)`. For example, if the current version is 4.0.1-beta-0014 and we want to release 4.0.1-beta-0005 (because the last version released was 4.0.1-beta-0004), and the `versionHeightOffset` is set to -21:
...
Go to the [GitHub Releases Page](https://github.com/NightOwl888/NetFx.Polyfills/releases) and click on the "Draft a new release" button.
...
3. Added tests for .NET Framework 4.8.1
```
...
To upload to NuGet.org, login to the [Azure DevOps Release Pipeline](https://dev.azure.com/NightOwl888/NetFx.Polyfills/_release). 
...
1. Open the [GitHub Releases Page](https://github.com/NightOwl888/NetFx.Polyfills/releases)
