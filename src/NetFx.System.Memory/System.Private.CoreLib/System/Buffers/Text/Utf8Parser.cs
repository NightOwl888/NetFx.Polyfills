    ...
    public static class Utf8Parser
    {
        ...
        label_17:
            ...
            label_24:
                ...
                if (start2 == 0)
                {
                    for (; start3 < index && source[start3] == (byte)48; ++start3)
                        --number.Scale;
                }
                int length2 = Math.Min(index - start3, 51 - start2 - 1);
                readOnlySpan = source.Slice(start3, length2);
            ...
            int start4 = index + 1;
            if ((options & ParseNumberOptions.AllowExponent) == (ParseNumberOptions)0)
            {
                bytesConsumed = 0;
                return false;
            }
            if (start4 == source.Length)
            {
                bytesConsumed = 0;
                return false;
            }
            bool flag = false;
            switch (source[start4])
            {
                case 43:
                    ++start4;
                    if (start4 == source.Length)
            ...
            uint num8;
            int bytesConsumed1;
            if (!TryParseUInt32D(source.Slice(start4), out num8, out bytesConsumed1))
            {
                bytesConsumed = 0;
                return false;
            }
            int num9 = start4 + bytesConsumed1;
            if (flag)
            {
                if ((long)number.Scale < (long)int.MinValue + (long)num8)
                    number.Scale = int.MinValue;
                else
                    number.Scale -= (int)num8;
            }
            else
            {
                if ((long)number.Scale > (long)int.MaxValue - (long)num8)
                {
                    bytesConsumed = 0;
                    return false;
                }
